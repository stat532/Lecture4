...
Example. Consider making comparisons between two properties of a distribution. For example a simple contrast, $\gamma = \theta_1 - \theta_2$, or a more complicated function of $\theta_1,$ and $\theta_2$ such as $\gamma = \log(\frac{\theta_1}{\theta_2})$. 
\vfill
...
\vfill
These procedures are both examples of Monte Carlo procedures to approximate integrals.
\vfill
\newpage
...
\vfill
There are three main uses for Monte Carlo procedures: 
\vfill
...
In the previous section, we saw that a beta distribution was a conjugate prior for a sampling model, meaning that the posterior was also a beta distribution. This prior specification allows easy posterior computations because 
\vfill

In many situations, this type of prior is not available and we need to use other means to understand the posterior distribution $p(\theta|y)$. 
\vfill
